...
// Not a test: sets up MeleeCombat's opening - a revealed map, and a second player whose Warrior and
// Settler start a couple of tiles from yours - then hands the game over to play by hand. The second
// player never acts, and ends its turn as soon as each one starts, so Next Turn works as normal.
export function setupCombatSandboxTest(game: Game) {
    ...
    runner.addStep({
        ...
        action: async () => {
            await utils.startGameWithSecondPlayer({ autoEndTurns: true });
        },
    ...
    runner.addStep({
        ...
        action: async () => {
            await utils.waitUntil(
                () => {
                    warrior = utils.getClientPlayer().getUnits().find((unit) => unit.canFight());
                    return !!warrior;
                },
                10000,
